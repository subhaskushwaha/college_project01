...
const AdminDashboard = () => {
    ...
    const loadAgents = async () => {
        try {
            ...
            if (!token) return;
            const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/admin/agents`, 
    ...
    const handleAddAgent = async (e) => {
        ...
        try {
            const token = localStorage.getItem("token");
            const response = await fetch(`${process.env.REACT_APP_API_URL}/api/admin/agents`, {
                method: "POST",
    ...
    const handleEditAgent = async (e) => {
        ...
        try {
            const token = localStorage.getItem("token");
            const response = await fetch(`${process.env.REACT_APP_API_URL}/api/admin/agents/${agentForm.id}`, {
                method: "PUT",
                headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
    ...
    const handleDeleteAgent = async (id) => {
        ...
        try {
            const token = localStorage.getItem("token");
            const response = await fetch(`${process.env.REACT_APP_API_URL}/api/admin/agents/${id}`, {
                method: "DELETE",
                headers: { Authorization: `Bearer ${token}` },
            });
            const data = await response.json();
    ...
    const uploadLeads = async () => {
        ...
        try {
            ...
            // If there's an assigned agent, append agentId and action="assign"? 
            // The backend "upload" action just processes it. You'd need a separate mechanism if you wanted to assign immediately according to mergedLeadController, or we just upload for now.
            
            const token = localStorage.getItem("token");
            await axios.post(`${process.env.REACT_APP_API_URL}/api/leads/lead-operation`, formData, {
                headers: { 
        ...
        <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
            ...
            <main className="container mx-auto px-4 py-8 flex-1 w-full max-w-7xl">
                ...
                {currentPage === 'manage-leads' && (
                    <ManageLeads getStatusBadge={getStatusBadge} agents={agents} />
                )}
                {currentPage === 'reports' && (
